import random
import numpy as np
from config import BLOCK_SIZE, W, H, Point, SENSE
...
class Agent_D:
    def __init__(self, env,id,en = 100):
        ...
        self.branin = Linear_QNet(53, 256, 5)
    ...
    def reset(self):
        ...
        self.pos = Point(x,y)
    def get_state(self, memory=[0]*29):
        state = self.env.get_state_agent_test(self.id, memory,SENSE)
    ...
    def take_action(self,state):
        ...
        # print(reward)
        self.death = death
        state_new = self.get_state(action+list(state[:24]))
        # check death
        if(self.en < 0):
            self.death = True
